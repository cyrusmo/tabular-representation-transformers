...
class _BaseTabularState(BaseEstimator):
    def __init__(self, d_token: int = 64, max_epochs: int = 20, lr: float = 1e-3, batch_size: int = 256, random_state: int = 42):
        self.d_token = d_token; self.max_epochs = max_epochs; self.lr = lr; self.batch_size = batch_size; self.random_state = random_state
    def _fit_torch(self, X, y, task: str, n_classes: int = 2):
        ...
        self.model_ = TabularStateTransformer(TabularStateConfig(n_features=X_t.shape[1], d_token=self.d_token, task=task, n_classes=n_classes))
        ...
        for _ in range(self.max_epochs):
            for xb, yb in loader:
                optim.zero_grad(); loss = loss_fn(self.model_(xb), yb); loss.backward(); optim.step()
        return self
